...
# Seleccionar interfaz de red de forma numérica
function seleccionar_interfaz() {
    interfaces=($(iw dev | awk '$1=="Interface"{print $2}'))

    if [[ ${#interfaces[@]} -eq 0 ]]; then
        echo -e "$rojo[!] No se encontraron interfaces inalámbricas disponibles.$reset"
        exit 1
    fi

    echo -e "$azul[*] Interfaces disponibles:$reset"
    for i in "${!interfaces[@]}"; do
        echo -e "${verde}$((i+1))${blanco}) ${interfaces[$i]}"
    done

    echo -e "$blanco Selecciona el número de la interfaz:"
    read -rp " #> " num

    if [[ "$num" =~ ^[0-9]+$ ]] && (( num >= 1 && num <= ${#interfaces[@]} )); then
        antena="${interfaces[$((num-1))]}"
        echo -e "$cian[+] Interfaz seleccionada: $antena$reset"
    else
        echo -e "$rojo[!] Selección inválida. Intenta de nuevo.$reset"
